...
export default function UserDashboard() {
  ...
  const loadClubSubscriptions = async () => {
    try {
      const response = await fetch('https://event-hub-backend-3.onrender.com/api/user/subscriptions', {
  ...
  const handleClubSubscription = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://event-hub-backend-3.onrender.com/api/payments/club-subscription', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          phone_number: phoneNumber,
